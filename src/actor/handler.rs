...
mod tests {
    ...
    fn test_typed_handler_dispatch() {
        ...
        let handler = TypedHandler::<IncrMsg, _>::new(move |state, msg, _reply| {
        ...
        handler.handle(&mut state, &payload, None);
        handler.handle(&mut state, &payload, None);
    ...
    fn test_typed_handler_with_reply() {
        let handler = TypedHandler::<IncrMsg, _>::new(|state, msg, reply| {
            let counter = state.get_mut::<u32>().unwrap();
            *counter += msg.amount;
            if let Some(reply) = reply {
        ...
        handler.handle(&mut state, &env.payload, env.reply);
    ...
    fn test_typed_handler_decode_error() {
        let handler = TypedHandler::<IncrMsg, _>::new(|_state, _msg, _reply| {
            panic!("should not be called on bad payload");
        });

        let mut state = ActorState::new();
        // Bad payload — too short
        let status = handler.handle(&mut state, &[0u8, 1], None);
