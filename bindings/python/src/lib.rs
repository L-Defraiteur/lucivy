...
fn collect_sharded_results(
    ...
    // caller asked for the fields (then all of them, in parallel past a
    // threshold — `ShardedHandle::fetch_docs`).
    let ids = handle.node_ids_of(results).map_err(PyValueError::new_err)?;
    let docs: Option<Vec<LucivyDocument>> = if include_fields {
        Some(handle.fetch_docs(results).map_err(PyValueError::new_err)?)
    } else {
        None
    };
