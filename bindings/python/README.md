...
- **`search(..., fields=False)` no longer reads any document**: the id comes from the fast field; with `fields=True` the documents are read in parallel past 64 hits (5 202 documents in 15 ms instead of 114, a top-200 in 0.6 ms).
