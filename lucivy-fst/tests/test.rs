...
use lucivy_fst::automaton::{Str, Subsequence};
#[cfg(feature = "levenshtein")]
use lucivy_fst::raw::{Builder, Fst};
use lucivy_fst::set::Set;
use lucivy_fst::{self, Automaton, IntoStreamer, Streamer};
...
fn implements_default() {
    let map: lucivy_fst::Map<Vec<u8>> = Default::default();
    assert!(map.is_empty());

    let set: lucivy_fst::Set<Vec<u8>> = Default::default();
    assert!(set.is_empty());
}
