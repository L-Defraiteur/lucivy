//! Finite state transducers for the lucivy search engine.
//!
//! Fork of BurntSushi/fst (MIT/Unlicense) with multi-output support
//! and optimizations for code-source indexing.
//!
//! Provides `Map` (key→u64), `Set` (key only), and `raw::Fst` (core FST).
//! Keys are byte strings, inserted in lexicographic order.
//! Supports automaton-based search (Levenshtein, regex, prefix, custom).
