...
impl<D: AsRef<[u8]>> Map<D> {
    ...
    ///
    /// ```rust
    /// use lucivy_fst::{IntoStreamer, Streamer, Map};
    ///
    /// let map = Map::from_iter(vec![("a", 1), ("b", 2), ("c", 3)]).unwrap();
    ...
    ///
    /// ```rust
    /// use lucivy_fst::automaton::Subsequence;
    /// use lucivy_fst::{IntoStreamer, Streamer, Map};
    ///
...
"##
    ...
    ///
    /// ```rust
    /// use lucivy_fst::{Streamer, Map};
    /// use lucivy_fst::map::IndexedValue;
    ///
...
///
/// ```rust
/// use lucivy_fst::{IntoStreamer, Streamer, Map, MapBuilder};
///
