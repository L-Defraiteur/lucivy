...
- Temps d'indexation de référence (noyau, index neufs) : **dictionnaire 39,9 s le 13 septembre dans la nuit (branche `v4.3` : `.pidx` puis textes en attente par époque de commit — `minted` 22 539 376, `pending` 6 570 578, 308 segments, à comparer à chaque A/B), 47,4 s avec le seul `.pidx`, 48,2 s au soir** (97,4 le matin même : plafond de paires 64, cache partagé vérifié, compaction et replis en pipeline sans allocation par clé — 10,5 → 3,0 s —, puis 16 fils d'indexation à budgets par fil constants, 308 segments, requêtes égales ; `docs/13-09-2026/07`) ; avant : v3 56 s, dictionnaire 131 s le 5 au soir →
