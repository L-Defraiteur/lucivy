...
`deferred_fold_settles`. **Cache partagé des ids trouvés** (`dictionary::LookupCache`, 13 septembre) : table fixe de paires atomiques sans verrou, chaque hit vérifié sur `.termtexts` avant usage ; noyau 97 → 66 s, `docs/13-09-2026/07-indexation-profil.md`. Le chemin par jeton a aussi été allégé (lecteurs
