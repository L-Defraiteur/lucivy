...
`LUCIVY_DICT_MAX_PENDING` (**64 depuis le 13 septembre** : à 16, chaque commit de 10 000 fichiers du noyau repliait en synchrone, 25 s des 97) et `LUCIVY_DICT_SYNC_FOLD=1` ; test
`deferred_fold_settles`. **Cache partagé des ids trouvés** (`dictionary::LookupCache`, 13 septembre) : table fixe de paires atomiques sans verrou, chaque hit vérifié sur `.termtexts` avant usage ; noyau 97 → 66 s, `docs/13-09-2026/07-indexation-profil.md`. **Index dérivé des groupes de parents** (`dict-<g>.<champ>.pidx`, `suffix_fst/dictionary_pidx.rs`, 13 septembre tard, branche `v4.3`) : un point de contrôle par 16 groupes des records de plus de 16 groupes, écrit par compactions et replis, reconstruit en RAM s'il manque, ignoré par un ancien lecteur (format 8 intact) ; noyau 48,2 → 47,4 s, décodage des parents 52 → 23 s de CPU ; vérifié dans Chrome sur 10 000 fichiers (un `.pidx` par génération et par shard, comptes et spans de l'après-midi) et sur l'index Linux 2.6.0 écrit par la 4.2.0 rouvert sans le fichier ; **tout fichier ajouté à une génération se déclare dans `dictionary::GENERATION_EXTENSIONS`** (inventaire du GC, snapshots, `remove_leftovers`, tailles, banc) ; `07` § 5 sexies. Le chemin par jeton a aussi été allégé (lecteurs
...
- Temps d'indexation de référence (noyau, index neufs) : **dictionnaire 35 s le 13 septembre dans la nuit (branches empilées de `v4.3` : `.pidx` 47,4, textes en attente par époque de commit 39,9, collecteur sans allocation par occurrence 35 — `minted` 22 539 376, `pending` 6 570 578, 308 segments, à comparer à chaque A/B, alterné deux fois), 48,2 s au soir** (97,4 le matin même : plafond de paires 64, cache partagé vérifié, compaction et replis en pipeline sans allocation par clé — 10,5 → 3,0 s —, puis 16 fils d'indexation à budgets par fil constants, 308 segments, requêtes égales ; `docs/13-09-2026/07`) ; avant : v3 56 s, dictionnaire 131 s le 5 au soir →
...
**Pour repartir (14 septembre au matin, 4.3.0 publiée)** : `docs/14-09-2026/01-rapport-nuit-4.3.0.md`, puis `docs/13-09-2026/09-architecture.md` et `10-knowledge-dump.md` (à jour au fil de la nuit). **Chantiers du 13 au soir, tard (branches empilées `v4.3` → `4.3.0.1-commit-path` → `4.3.0.2-minted-pair`, publiés en 4.3.0)** : le `.pidx`, `docs/13-09-2026/07-indexation-profil.md` § 5 sexies — mesure de la forme des records (2,9 % groupés, 79 % des parents), index à points de contrôle, cycle de vie du fichier, A/B contre 4.2.0 rebâtie dans un arbre à part ; puis **§ 5 septies, le chemin sériel du commit** — `benches/gdb_timeline.py` (le pool dans le temps : vallées à un fil occupé à chaque commit), textes en attente par époque de commit (`DictionaryShared.epoch`, `begin_commit_epoch` dans `prepare_commit`, `forget_committed_pending`), plus de `HashSet` d'ids repliés, noyau 48,2 → 39,9 s à ids et segments identiques ; le bug de hachage attrapé par le compteur `minted` ; puis **§ 5 octies, le collecteur** — une entrée de mot par ordinal, intervalles au lieu de `BTreeMap`, tampons réutilisés, champ mort `content_overlap` retiré, noyau 39 → 35 s, contrôle à l'octet (dictionnaire identique ; en v3 le record d'un mot porte la forme de la première occurrence, celle de `.termtexts`, `diff_two_sfx_files`), panel de parité Chrome 20/21 + 21/21 ; puis **deux défauts d'exactitude corrigés, présents dans 4.0.0 à 4.2.0** (§ 5 nonies) : la coupe d'une liste de candidats par le `.gmap` d'un segment petit face à la liste gardait un item par id (les occurrences répétées d'une aiguille dans un jeton perdues selon la forme des segments — `de` sur le noyau à 24 fils d'écriture), et le relâché rendait des spans finissant dans un caractère multi-octets (recouvrement de contenu pris sur le mot d'après quand le suivant commence par un caractère de 3 octets, et candidats ancrés de la partition mot-dépouillé non filtrés à `sti` 0) ; tests `keep_in_segment_keeps_every_item_of_a_repeated_id`, `test_relaxed_multibyte` ; **§ 5 octies bis** : textes en attente sondés avant les parties (marches FST 68 → 38 s de CPU) ; parité Chrome 21/21 dans les deux dispositions ; **§ 5 decies** : la paire dans son propre nœud du DAG, essayé et retiré (rien de mesurable) ; **§ 5 undecies** : la paire mintée renommée `.minted.termtexts` / `.minted.sfx` (les deux noms lus). Comparatif régénéré pour 4.3.0 : `docs/compare-engines-2026-09-14.md` (lucivy 29-35 s, tailles égales). **Pour repartir (13 septembre au soir, 4.2.0 publiée)** : `docs/13-09-2026/08-rapport-session-soir.md` (la demi-journée : document store, fetch parallèle, indexation 97 → 48 s, compaction, fils, comparatif régénéré, la 4.2.0 et son incident crates.io), puis `09-architecture.md` et `10-knowledge-dump.md` (autonomes, à jour ; le matin dans `03`, `04`, `05`). **Indexation profilée le 13 septembre** : `docs/13-09-2026/07-indexation-profil.md` — noyau 97 → 66 s (plafond de paires 64, cache partagé vérifié), profileur gdb `benches/gdb_sample.sh` + `gdb_top.py`, puis **compaction du dictionnaire 8,4 → 2,6 s** (banc `bench_dict_compaction.rs`, sortie identique à l'octet), puis **16 fils d'indexation** à budgets par fil constants (segments de même forme, requêtes égales), noyau 48 s ; ce qui reste (marche des parents, finalisation en ligne, arrêt du monde au commit). **Pour repartir (13 septembre)** : `docs/13-09-2026/03-rapport-session.md` (la journée : 4.1.0 publiée, le défaut des doublons, la CI refaite, le « 70 short » qui était notre motif, le banc épinglé, les spans à la demande), puis `04-architecture.md` et `05-knowledge-dump.md` (autonomes ; §5 de l'architecture ouvrait la piste du **document store** ; **mesurée dans `06-document-store.md`** : les 123 ms étaient 147 Mo de texte relus en entier par le harnais, la décompression LZ4 (48 ms) et la première touche du mmap — pas le cache de 4 blocs, pas le champ ; la vérification sans positions est bien parallèle par segment ; ce qui reste : les bindings relisent le document entier pour lire `_node_id`, un fast field, et le fetch des champs est séquentiel — **corrigé le même jour** : `_node_id` par fast field partout, `ShardedHandle::fetch_docs` parallèle par (shard, segment) via luciole au-delà de 64 hits, 114 → 15 ms sur 5 202 hits, vérifié sur 10 000 fichiers dans Chrome ; banc `bench_docstore_fetch.rs`, test `test_fetch_docs`). **Vision notée le 13 septembre** : `docs/13-09-2026/01-index-a-la-carte.md` — déclarer à la création les questions qu'on veut pouvoir poser (`features: ["contains:relaxed", "spans", …]`) et laisser le moteur choisir la forme de stockage la plus petite qui y répond exactement ; catalogue question → structures, règles à ne pas casser, étapes. **Pour repartir : `docs/11-09-2026/01-recap-session-index-sans-positions.md` puis `02-knowledge-dump.md` et `03-architecture.md`.** **Correction publiée le 12 septembre** : `docs/12-09-2026/01-elasticsearch-regex-casse.md` — les « 70 documents manquants » d'Elasticsearch sur la ligne regex du banc venaient de notre motif, pas du champ `wildcard` (`case_insensitive` de Lucene replie les littéraux, pas les classes) ; vérifié sur les octets stockés par Elasticsearch lui-même, corrigé dans les README, la page, l'article, le rapport et le script du banc. **Défaut du moteur publié trouvé et corrigé le 11 au soir** (4.0.2 comprise) : `04-doublons-de-spans-en-relache.md` — en relâché, un span en double quand l'aiguille termine un mot découpé en morceaux (`lock` dans `superblock`), tf et score gonflés ; déduplication par octets (`orchestrator::dedup_occurrences`), le harnais compte maintenant les doublons (il comparait des ensembles), test `test_relaxed_duplicate_spans`. **Chantier en cours (branche `v4.1`, depuis le 8 septembre)** : `docs/08-09-2026/01-chantier-positions-optionnelles.md`
...
**4.3.0 publiée le 14 septembre 2026 au petit matin** (tag `v4.3.0`, `main` = `8e47953`, PR #19 depuis les branches
empilées `v4.3` → `4.3.0.1-commit-path` → `4.3.0.2-minted-pair`, rebase) : indexation 48 → 35 s à index égal, deux
correctifs d'exactitude présents depuis 4.0.0, la paire mintée renommée ; tout est sorti du premier coup (PyPI 5 wheels +
sdist, npm ×6, `lucivy-wasm`, crates.io ×5, `lucivy-fst` inchangé en 0.1.1, release GitHub 12 artefacts). Autorisation de
Lucie avant de dormir, conditionnée à tout vert et tout à jour (README ×5, ARCHITECTURE.md, article, page, comparatif
régénéré, tableau navigateur remesuré). Rapport : `docs/14-09-2026/01-rapport-nuit-4.3.0.md`.
...
republie que ce qui manque. Le premier dispatch a été **refusé par l'environnement** (`release` n'autorisait que
les tags `v*` comme cible de déploiement : trois jobs en échec en 1 s sans étape, crates.io sauté) ; depuis, `main` est
aussi une branche de déploiement autorisée (`gh api -X POST repos/…/environments/release/deployment-branch-policies
-f name=main -f type=branch`), et **le chemin de republication partielle est** : `gh workflow run release.yml --ref
main -f publish=true` — chaque job saute ce qui existe déjà, « attach to the release » ne tourne que sur un tag. Leçon :
**un crate modifié doit changer de numéro avant le tag**, même la fourche FST.
...
|----------|---------|---------|---------|
| PyPI | `lucivy` | **4.3.0** (14 septembre 2026), **4.2.0** (13 septembre 2026 au soir), **4.1.0** (13 septembre 2026), **4.0.0**, **4.0.1**, **4.0.2** (5 wheels `cp39-abi3` : manylinux_2_28 x86_64 + aarch64, macOS x86_64 + arm64, win_amd64 ; + sdist) — par le tag | 6 septembre 2026 (nuit, puis 14 h 30) |
| npm | `lucivy` + `lucivy-linux-x64-gnu`, `lucivy-linux-arm64-gnu`, `lucivy-darwin-x64`, `lucivy-darwin-arm64`, `lucivy-windows-x64` | **4.3.0**, **4.2.0**, **4.1.0**, **4.0.0**, **4.0.1**, **4.0.2** — par le tag | 6 septembre 2026 (nuit, puis 14 h 30) |
| npm | `lucivy-wasm` | **4.3.0**, **4.2.0**, **4.1.0**, **4.0.0**, **4.0.1**, **4.0.2** (job `wasm` + `publish-wasm`) | 6 septembre 2026 (nuit, puis 14 h 30) |
| crates.io | `ld-lucivy`, `lucivy-core`, `luciole`, `lucistore`, `sparse-vector` (+ `lucivy-fst` 0.1.x) | **4.3.0** (14 septembre, par le tag, d'un coup), **4.2.0** (le 13 septembre au soir : `luciole` et `lucistore` par le tag, `lucivy-fst` 0.1.1, `ld-lucivy`, `lucivy-core` et `sparse-vector` par le dispatch de rattrapage), **4.1.0**, **4.0.0**, **4.0.1**, **4.0.2** — par le tag, en dernier | 6 septembre 2026 (nuit, puis 14 h 30) |
