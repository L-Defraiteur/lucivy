...
| tantivy 0.25, default tokenizer | it does not (whole words) | 654 MB | ×0.7 | 1 s |
| tantivy 0.25, `NgramTokenizer` (trigrams) | trigram phrases (positions all 0: candidates only) | 733 MB | ×0.8 | 5 s |
| lucivy 4.3, a dictionary per segment (`sfx_version` 3) | suffix FST, exact spans | 6 884 MB | ×7.7 | 32 s |
| lucivy 4.3, shared dictionary per shard | suffix FST, exact spans | 5 090 MB | ×5.7 | 35 s |
| lucivy 4.3, shared dictionary + `derived_in_ram` | suffix FST, exact spans | 3 434 MB | ×3.8 | 35 s |
| lucivy 4.3, shared dictionary + `positions: false` | suffix FST, exact spans | 2 517 MB | ×2.8 | 29 s |

## 2. The nine verified queries

| query | mode | truth (scan) | lucivy | spans | lucivy | lucivy, `positions: false` | Elasticsearch | tantivy |
|---|---|---|---|---|---|---|---|---|
| `mutex_lock` | substring | 5 202 | **5 202** OK | 21 070 | 13 ms | 26 ms | **5 202** · 1 ms (+403 ms with highlights on 200) | 5 229 · 107 ms |
| `mutex_lock` | separators relaxed | 5 862 | **5 862** OK | 23 067 | 13 ms | 36 ms | — | — |
| `spin_lock` | substring | 6 527 | **6 527** OK | 34 436 | 14 ms | 32 ms | **6 527** · 1 ms (+277 ms with highlights on 200) | 6 563 · 117 ms |
| `sched` | whole word | 5 246 | **5 246** OK | 27 341 | 22 ms | 53 ms | 1 724 · 1 ms (+116 ms with highlights on 200) | 5 260 · 0 ms |
| `sched` | substring | 9 214 | **9 214** OK | 52 228 | 13 ms | 38 ms | 9 181 · 0 ms (+201 ms with highlights on 200) | 9 238 · 148 ms |
| `printk` | start of token | 4 446 | **4 446** OK | 24 702 | 16 ms | 35 ms | 3 164 · 1 ms (+132 ms with highlights on 200) | 4 399 · 0 ms |
| `schdule` | fuzzy, 1 edit | 5 148 | **5 148** OK | 18 549 | 53 ms | 194 ms | 1 531 · 0 ms (+170 ms with highlights on 200) | 3 721 · 4 ms |
| `regsiter` | fuzzy, 2 edits | 34 833 | **34 833** OK | 265 247 | 880 ms | 373 ms | 21 165 · 1 ms (+461 ms with highlights on 200) | 29 438 · 16 ms |
| `spin_lock_[a-z]+` | regex | 5 471 | **5 471** OK | 24 156 | 257 ms | 21 ms | **5 471** · 4 ms (+1167 ms with highlights on 200) | 0 · 0 ms |
...
|---|---|---|---|---|
| `spin_lock`, separators strict | 6 527 | **6 527** OK, 34 436 spans, 14 ms | **6 527** (spin_lock, separators strict, 0 ms +166 ms with highlights on 200) | 6 563 (spin_lock (substring), 117 ms)<br>6 563 (spin_lock (trigrams, verified, strict), 117 ms) |
| `spin_lock`, separators relaxed — also `spin lock`, `spin-lock`, `spinlock` | 9 545 | **9 545** OK, 54 680 spans, 23 ms | 6 524 (spin_lock, separators relaxed (spin_lock, spin lock, spin-lock, spinlock), 0 ms +138 ms with highlights on 200)<br>170 ("spin lock" as a phrase, standard analyzer, 0 ms +171 ms with highlights on 170) | 6 608 (spinlock (trigrams, verified; must find spin_lock too), 113 ms)<br>6 594 ("spin lock" (phrase, default tokenizer), 1 ms) |
| `spinlokc`, two edits, across the token boundary | 10 117 | **10 117** OK, 56 836 spans, 167 ms | 3 534 (spinlokc, two edits, across the token boundary, 0 ms +463 ms with highlights on 200) | 6 585 (spinlokc (fuzzy, 2 edits, across the boundary), 17 ms) |
| `spin_lock_[a-z]+`, a regex | 5 471 | **5 471** OK, 24 156 spans, 257 ms | **5 471** (spin_lock_[a-z]+ (regex, wildcard field, case folded), 3 ms +846 ms with highlights on 200) | 0 (spin_lock_[a-z]+ (regex, terms), 0 ms) |
| `ude`, three characters | 74 500 | **74 500** OK, 478 423 spans, 100 ms | 68 561 (ude (three characters), 0 ms +92 ms with highlights on 200) | 74 679 (ude (three characters), 0 ms) |
| `de`, two characters | 100 166 | **100 166** OK, 7 929 772 spans, 588 ms | 0 (de (two characters), 1 ms +0 ms with highlights on 0) | 0 (de (two characters), 0 ms) |
| `retur -ENOMEM`, a fuzzy phrase (one edit: a letter missing) | 14 377 | **14 377** OK, 31 937 spans, 31 ms | 14 374 (retur -ENOMEM (fuzzy phrase: span_near of a fuzzy span and a term), 0 ms +151 ms with highlights on 200) | — |
...
| tantivy, AND of trigrams verified on the stored text, occurrences in the first 200 | 5 229 | 753 | 200 | 95 ms (the whole path) |
